...
class DataSplit(Enum):
    ...
    def get_from_value(self, value):
        for item in DataSplit:
            if item.value == value:
                return item
        raise ValueError(f"Value {value} not found in DataSplit enum")
...
class RetrievalStore:
    ...
    def _document_exists(self, idx):
        # Query the ChromaDB collection based on metadata
        results = self.collection.get(ids=[str(idx)])
    ...
    def _add_document(
        ...
        if not self._document_exists(idx):
            self.collection.add(
                documents = [document],
